...
import com.liu.studentmanager.service.ClazzService;
import com.liu.studentmanager.service.CourseService;
import com.liu.studentmanager.service.ScoreService;
...
//                0
                ...
                //将学生，课程转换为id,存入数据库
                // 1)首先获取对应的id
                int clazzId= clazzService.findByClazz(row.getCell(0).getStringCellValue());
                int studentId = studentService.findByName(row.getCell(1).getStringCellValue());
                int courseId = courseService.findByName(row.getCell(2).getStringCellValue());
                // 2)判断是否已存在数据库中
